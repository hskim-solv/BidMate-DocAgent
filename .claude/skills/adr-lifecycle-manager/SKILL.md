...
4. **편집 + 동기화** — 선택된 해소를 ADR Status 블록에 적용. Status 문자열이 바뀌면 [docs/adr/README.md](../../../docs/adr/README.md) 인덱스 row의 status 컬럼도 **같은 커밋**에서 갱신. **status 컬럼 일치는 이제 자동 검증된다**: `python3 scripts/_governance.py --check-adr-readme-status`(#1181) 가 각 ADR 파일의 Status 헤더를 메인 인덱스 row 의 status 셀과 (leading lifecycle word 기준) 대조하고, CI 는 `test_adr_readme_main_index_status_matches_files` 로 같은 검사를 한다 — proposed→accepted flip 후 row 가 stale 하면 둘 다 red. 편집 후 이 검사를 돌려 green 인지 확인. (`--check-adr-readme-parity`(#803) 는 여전히 row **누락**[파일명] 검출 — status 와 별개 차원이라 둘 다 실행.) **수동 확인이 남는 건 단 하나**: promote 시 README "Roadmap / Promote 조건" 표에서 해당 row 를 빼는 것 — 그 표는 status 컬럼이 없어 `--check-adr-readme-status` 범위 밖(메인 인덱스만 검사).
5. **로컬 검증 (gate)** — `bash scripts/test.sh`(특히 `test_no_unlinked_adr_files_on_disk` [row 멤버십] + `test_adr_readme_main_index_status_matches_files` [status 컬럼, #1181] + collector 테스트). post-2026-05-15 ADR 편집 시 `--lint-adr-consequences` 통과 확인. commit `chore(adr): resolve proposed SLA — <NNNN> <action> (closes #N)`(다건이면 요약).
...
- **"README parity 무시"** → 거부, 단 정확히. row **누락**(파일명 미링크)은 `test_no_unlinked_adr_files_on_disk` CI를 머지 시 red로 만들어 모든 open PR에 cascade(이슈 #730/#732/#750 전례) — 그러므로 row 자체는 반드시 추가. **status 컬럼 불일치도 이제 CI(`test_adr_readme_main_index_status_matches_files`) + `--check-adr-readme-status`(#1181) 가 잡는다** — 과거엔 수동 readback 이 유일 방어선이었으나 #1181 로 자동화됨. 남은 수동 확인은 promote 시 "Promote 조건" 표 row 제거뿐(그 표엔 status 컬럼이 없어 검사 범위 밖).

## References

- ADR 0047 — 1인 저자 ADR governance: 30일 proposed SLA + Verification 계약 + 번호예약.
- [scripts/_governance.py](../../../scripts/_governance.py) — `--proposed-adr-age`(PR #1094 탐지 + #1178 `## Resolution` H2 인식 → `resolved_in_place`), `--lint-adr-consequences`(#793), `--check-adr-readme-parity`(#803, row 파일명 존재[멤버십] 검증), `--check-adr-readme-status`(#1181, 메인 인덱스 row 의 status 컬럼 ↔ ADR 파일 Status 헤더 parity; leading lifecycle word 기준), `--next-adr-number`(다음 번호 **힌트, 예약 아님**; 동시 worktree 미감지).
