...
| 29 | `T-2026-0029` | `review` | Implementer -> Benchmark Auditor -> Privacy Auditor -> Reviewer | issue #1764; page-aware re-measurement DONE (diagnostic-only). Page blocker resolved (coverage 0.0 -> 1.0) but doc-level retrieval collapsed (~12%); renderer BUG #1/#2 hardened; root cause spun off as T-2026-0075. |
...
| 72 | `T-2026-0072` | `done` | Implementer -> Reviewer | 2026-06-01: agent-loop 보조도구 4종 advisory-only(호출만) 통합 완료. T-X3 ralplan plan-gate (PR #1743 / issue #1741), T-X2 eval-anomaly-investigator (#1744 / #1742), T-X1 wiki+memory-curator learning-capture (#1747 / #1745), T-X4 codex:rescue/tracer repair-escalation (#1748 / #1746) — 전부 제어 흐름 불변(preflight exit code · gate ready 결정 · auto-loop defer/stop 그대로), EXECUTE_SHIP=0 유지, 각 통합=별도 worktree + ADR 0007 브랜치 + 회귀 테스트. #1748 은 #1747 과 `write_active_auto_loop` helper 위치 충돌 → 두 advisory 모두 보존 해소 후 CI green 머지. plan 문서 아카이브: PR #1750 (issue #1749), [`docs/plans/agent-loop-integration-plan.md`](../docs/plans/agent-loop-integration-plan.md). |
...
| 75 | `T-2026-0075` | `ready` | Implementer -> Benchmark Auditor -> Privacy Auditor -> Reviewer | P0; spun off from T-2026-0029 page-aware re-measurement. MiniLM page-aware `real100_v2` rebuild resolved the page blocker but collapsed doc-level retrieval (gold-doc-in-retrieved 61.4% -> 12.1%) despite 88.3% universe coverage (answerable-with-gold; 91.7% all-cases); the diagnostic renderer's new `retrieval_integrity_suspect` signal points here. Gates every downstream `real100_v2` optimization task (T-2026-0030/0032/0033). Issue: TBD (created when task starts). |
...
- Reopened follow-up: the 2026-05-28 diagnostic conclusions were rerun against
  ...
  investigation is spun off as `T-2026-0075`.
...
- Decisions made: page-span blocker resolved (coverage 0.0 -> 1.0, 24613/24613 chunks); doc-level retrieval collapsed (gold-doc-in-retrieved 61.4% -> 12.1%, all-gold-observed rate ~0.7%); renderer now emits `retrieval_integrity_suspect` signal pointing at T-2026-0075 instead of the reranker (T-2026-0032) because the candidate pool itself collapsed; root-cause investigation spun off as T-2026-0075 (queue-only this PR, no GitHub issue yet). This PR is diagnostic-only — no retrieval/reranker/verifier/answer/ingestion/chunking/eval-scoring behavior changed.
- Commands run: REAL_EVAL_ROOT=/Users/hskim/Desktop/projects/BidMate-DocAgent python3 scripts/render_real100_v2_retrieval_diagnostics.py; python3 -m pytest -q tests/test_render_real100_v2_retrieval_diagnostics.py.
...
- Open questions: none (T-2026-0075 GitHub issue deferred to task start per maintainer decision).
```
...
## T-2026-0075 — Verify real100_v2 page-aware retrieval integrity (embedding parity + provenance)
