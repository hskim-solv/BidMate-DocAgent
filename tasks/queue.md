...
| 71 | `T-2026-0071` | `ready` | Implementer -> Security Reviewer -> Reviewer | issue #1703; P2.0/P2.1 landed (#1698/#1700/#1702, ADR 0088/0090/0091) + operator branch-protection DONE (integration+main, `protection_verified` VERIFIED). 2026-06-07 sync after #2700/#2699: manifest schema v2 + opt-in `agent_loop.py` emission seam + `make 시작-ship` serial `ACTIVE_TASK_POOL=1` wiring + read-only GitHub GraphQL review-gate resolver are now merged. Remaining P2.2 work is now narrowed to `SELF_IMMUTABLE_PATHS` SSoT/refusal wiring, cap-store redesign (ADR 0093+ / single-writer lane), staging live e2e during an integration operating window, and main promotion wiring/source-binding policy. Deep context: issue #1703 + #2717, PR #2700, plan #1708, [`docs/plans/T-2026-0071-p22-autonomous-merge-orchestration.md`](../docs/plans/T-2026-0071-p22-autonomous-merge-orchestration.md), and `docs/operations/staging-self-ship.md`. |
