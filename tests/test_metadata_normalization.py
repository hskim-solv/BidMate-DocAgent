...
def test_normalize_page_span_decimal_string_pair_falls_back_to_regions() -> None:
...
def test_normalize_page_span_whitespace_nonpositive_pair_falls_back_to_regions() -> None:
    assert normalize_page_span([" 0 ", "\t3\n"], [{"page_number": 5}]) == [5, 5]
    assert normalize_page_span(["\t-1\n", "3"], [{"page_number": 5}]) == [5, 5]


def test_normalize_page_span_float_pair_falls_back_to_regions() -> None:
...
def test_normalize_page_span_ignores_whitespace_decimal_string_region_pages() -> None:
    assert normalize_page_span(
        None, [{"page_number": " \t3.0\n"}, {"page_number": 5}]
    ) == [
        5,
        5,
    ]
