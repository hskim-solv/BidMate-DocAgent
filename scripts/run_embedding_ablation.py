#!/usr/bin/env python3
"""Embedding model ablation runner (issue #148).
...
The default model set is the README "Limitations" list (MiniLM-L12-v2 vs
multilingual-e5-base). Large models (BGE-M3 ~2GB, e5-large ~1.3GB) are
opt-in via ``--models`` to keep the default run cheap.
"""
...
def build_index(model_id: str, index_dir: Path) -> None:
    _run(
        [
            sys.executable,
...
def main() -> int:
    ...
    for model_id in args.models:
        ...
        if args.reuse_existing and summary_path.exists():
            ...
            print(f"\n[build] index for {model_id}", flush=True)
            build_index(model_id, index_dir)
