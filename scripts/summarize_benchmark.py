...
def render_docs(registry: dict[str, Any]) -> str:
    ...
    if comparison_rows:
        lines.extend(
            [
                ...
                "",
                "이 표는 공개 fixture smoke 결과와 익명 private aggregate를 함께 볼 때만 생성된다. private row는 원문, 파일명, 기관명, 질의 본문, 개별 답변, 실행 추적 없이 집계 지표만 사용한다.",
                "",
    ...
    lines.extend(
        [
            ...
            "- 현재 수치는 공개 fixture smoke RFP 평가셋 기준의 2차 가공 집계이며, 원본 RFP 문서나 raw example output은 포함하지 않는다.",
            "",
            "## Next Actions",
            "",
