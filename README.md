...
- **Latency** (`naive_baseline`, hashing 임베딩, macOS CPU, public synthetic n=42, retrieval+verify+answer 합 walltime): p50 1.7 ms / p95 5.3 ms. 측정 범위·콜드스타트 분리·임베딩 backend 차이는 [핵심 성능표](#핵심-성능표-실측) 본문 line 252–260 측정환경 블록 참고.
...
- **Orchestrator (opt-in)**: `BIDMATE_ORCHESTRATOR=langgraph`로 켜면 analyze / retrieve_loop / build_answer 3-노드 LangGraph StateGraph로 위 phase가 실행됨. direct 경로와 byte-identical JSON을 보장(회귀 [tests/test_langgraph_orchestrator_regression.py](tests/test_langgraph_orchestrator_regression.py)), `naive_baseline`은 ADR 0001로 인해 langgraph 경로를 우회. 설계는 [ADR 0022](docs/adr/0022-langgraph-orchestration-stage-1.md).
...
- **HWP native parse — v3 로드맵 작업 영역** — 본 항목은 **binary HWP**(`.hwp`, 한컴오피스 native format)에 한정. **HWPX**(`.hwpx`, OOXML 패키지 기반 한글 표준)는 현재 별도 native 파서가 없으며 다른 비표준 포맷과 동일하게 CSV `텍스트` 컬럼 fallback에 의존(코드 검색 시 `.hwpx` 핸들러는 없음 — `scripts/compare_hwp_extraction.py`는 binary HWP path만 다룸). binary HWP 기본 경로는 CSV `텍스트` 컬럼 fallback (`visual_fallback_hwp` 마커). 한국 정부조달 RFP 도메인에서 HWP 비율이 높으므로 v3 사이클에서 두 native 경로(`hwp5txt` 텍스트 추출 vs `libreoffice --headless` HWP→PDF → visual-v2)를 비교 측정 중. 비교 설계와 결과 기록: [`docs/hwp-extraction-comparison.md`](docs/hwp-extraction-comparison.md). 사촌 spike(`pyhwp` Python API): [`docs/hwp-native-spike.md`](docs/hwp-native-spike.md).
- **Embedding 디폴트 = MiniLM-L12-v2 ([ADR 0019](docs/adr/0019-embedding-default-stays-minilm.md) 잠금, [ADR 0021](docs/adr/0021-bge-m3-completes-phase-1-3.md) 보강)** — 1차 측정(MiniLM-L12-v2 vs multilingual-e5-base, 2026-05-11): full agentic 파이프라인에서는 0pp Δ, naive_baseline에서는 e5-base가 accuracy +18.8pp — metadata-first filtering이 임베딩 선택에 robust함을 실증. 2차 사이클 Phase 1.2 (이슈 #174, 2026-05-12) 3-of-4 측정 후 **Phase 1.3 (이슈 #389, 2026-05-12)** 에서 `BAAI/bge-m3` 측정 완료(`torch >= 2.6` 블로커 해소). 4개 named candidate 전체 측정 종료: `multilingual-e5-large-instruct` / `KoSimCSE-roberta-multitask` / `BGE-M3` 모두 `full` 파이프라인에서 0pp Δ (메트릭이 bit-identical) — 한국어 특화 + 2024 multi-functional SoTA까지 포함해 "0pp on full" 패턴이 falsify되지 않음. ADR 0019 재오픈 조건 3 (full에서 ≥+5pp + non-overlapping CI)은 4개 candidate 모두 미충족 → 디폴트 그대로 유지. 5개 임베딩(2019–2024 span) cross-architecture 측정 표·러너: [`docs/embedding-ablation.md`](docs/embedding-ablation.md).
- **External baseline 비교 프레임워크 구현 완료** — `scripts/compare_external_baselines.py` (ADR 0009) 구현 완료. *대칭 metric subset*(accuracy / retrieval_recall@k / latency)만 보고, 외부 system이 produce하지 않는 *비대칭 metric*(citation_precision / claim_citation_alignment / abstention_accuracy / answer_format_compliance)은 `null`로 명시 — 이게 "왜 자체 구축?" 질문에 대한 정량적 답변 자체. stub backend 실행 결과 `reports/external_baselines.json` 커밋 완료(n=42). 실 LangChain `RetrievalQA` / LlamaIndex `QueryEngine` 실행은 issue [#449](https://github.com/hskim-solv/BidMate-DocAgent/issues/449) — 해당 이슈가 완료되면 ADR 0025 re-open 조건 충족.
...
- **헤드라인 latency 기준 preset**: 위 요약표 `naive_baseline` 컬럼의 Latency p95 (5.3ms)는 CI source of truth. `agentic_full` 컬럼의 4.6ms는 ablation 표 측정값. `agentic_full_llm`은 LLM 서비스 레이턴시를 포함해 환경 의존적이므로 CI에서 고정하지 않으며 collapsed detection-blind 표에서만 확인합니다.
...
| System | Latency (p50/p95) | p50 2.2ms / p95 7.1ms (`agentic_full`) | p50 1.9ms / p95 5.9ms (`naive_baseline` — CI source of truth) | — |
...
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| naive_baseline | naive_baseline | 4 | off | off | off | 0.844±0.12 | 0.714±0.14 | 0.512±0.12 | 0.974±0.05 | 0.667 | 0.300 | 0.000 | 5.9ms |
| full | agentic_full | auto | on | on | on | 0.906±0.12 | 0.929±0.07 | 0.905±0.08 | 1.000±0.00 | 0.905 | 1.000 | 0.310 | 7.1ms |
...
| Run | Pipeline | Top-k | Metadata-first | Rerank | Verifier/Retry | Accuracy | Groundedness | Citation | Claim Align | Format | Abstention | Retry | Latency p95 |
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| full_llm | agentic_full_llm | auto | on | on | on | 0.906±0.12 | 0.929±0.07 | 0.905±0.08 | 1.000±0.00 | 0.905 | 1.000 | 0.310 | 5.1ms |
| full_llm_metadata | agentic_full | auto | on | on | on | 0.906±0.12 | 0.929±0.07 | 0.905±0.08 | 1.000±0.00 | 0.905 | 1.000 | 0.310 | 4.6ms |
...
| hybrid_bm25_k10 | agentic_full | auto | on | on | on | 0.906±0.12 | 0.929±0.07 | 0.905±0.08 | 1.000±0.00 | 0.905 | 1.000 | 0.310 | 4.8ms |
| hybrid_bm25_k30 | agentic_full | auto | on | on | on | 0.906±0.12 | 0.929±0.07 | 0.905±0.08 | 1.000±0.00 | 0.905 | 1.000 | 0.310 | 5.1ms |
| hybrid_bm25_k100 | agentic_full | auto | on | on | on | 0.906±0.12 | 0.929±0.07 | 0.905±0.08 | 1.000±0.00 | 0.905 | 1.000 | 0.310 | 6.4ms |
| hybrid_bm25_extra_stopwords | agentic_full | auto | on | on | on | 0.906±0.12 | 0.929±0.07 | 0.905±0.08 | 1.000±0.00 | 0.905 | 1.000 | 0.310 | 4.7ms |
| hybrid_bm25_k30_extra | agentic_full | auto | on | on | on | 0.906±0.12 | 0.929±0.07 | 0.905±0.08 | 1.000±0.00 | 0.905 | 1.000 | 0.310 | 7.9ms |
| full_kiwi | agentic_full | auto | on | on | on | 0.906±0.12 | 0.929±0.07 | 0.905±0.08 | 1.000±0.00 | 0.905 | 1.000 | 0.310 | 4.7ms |
| full_reranker | agentic_full | auto | on | on | on | 0.906±0.12 | 0.929±0.07 | 0.905±0.08 | 1.000±0.00 | 0.905 | 1.000 | 0.310 | 4.7ms |
| full_hyde | agentic_full | auto | on | on | on | 0.906±0.12 | 0.929±0.07 | 0.905±0.08 | 1.000±0.00 | 0.905 | 1.000 | 0.310 | 4.6ms |
| agentic_full_finetuned | agentic_full | auto | on | on | on | 0.906±0.12 | 0.929±0.07 | 0.905±0.08 | 1.000±0.00 | 0.905 | 1.000 | 0.310 | 5.1ms |
| naive_baseline_finetuned | naive_baseline | 4 | off | off | off | 0.844±0.12 | 0.714±0.14 | 0.512±0.12 | 0.974±0.05 | 0.667 | 0.300 | 0.000 | 3.1ms |
...
`sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2`를 기본으로 채택. 채택 이유: 한국어/영어 multilingual 지원, L12 경량(~120MB), API 의존 0, 로컬 재현성. **검증된 한계**: Phase 1.2 + **Phase 1.3** ablation (2026-05-12, n=42)에서 `multilingual-e5-large-instruct`, `KoSimCSE-roberta-multitask`, `BAAI/bge-m3` 셋 모두 `full` 파이프라인에서 0pp Δ — 메트릭이 bit-identical하게 일치. 임베딩 선택 robustness는 ADR 0002 metadata-first 설계의 empirical consequence임이 한국어 특화 + 2024 multi-functional SoTA까지 확장되어 falsify되지 않음 ([ADR 0019](docs/adr/0019-embedding-default-stays-minilm.md) + [ADR 0021](docs/adr/0021-bge-m3-completes-phase-1-3.md)).
