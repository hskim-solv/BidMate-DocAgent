...
| **어떻게 평가 (evaluated)** | baseline 보존 ablation ([ADR 0001](docs/adr/0001-preserve-naive-baseline.md)) + 공개 fixture smoke / private internal eval 분리 ([ADR 0005](docs/adr/0005-eval-split-public-synthetic-private-local.md)) + PR 마다 회귀 게이트 ([pr-eval.yml](.github/workflows/pr-eval.yml)) + 95 ADR |
...
| **Live demo (HF Spaces)** | [![Open in HF Spaces](https://huggingface.co/datasets/huggingface/badges/resolve/main/open-in-hf-spaces-sm.svg)](https://huggingface.co/spaces/hskim-solv/bidmate-docagent) | **정식 라이브 데모** — 가입·빌링 불필요. Space sleep 시 cold-start 30–60s. 운영: [`docs/operations/deployment.md`](docs/operations/deployment.md#hugging-face-spaces) |
| Fly.io (실험·보류) | — | **parked**: 자동 재배포 비활성 ([deploy-fly.yml](.github/workflows/deploy-fly.yml) `if: false`) — depot.dev 빌더 i/o timeout 으로 2026-05-26~ 연속 실패. 앱·[fly.toml](fly.toml) 보존(비용 ~$0), destroy 안 함 |
...
| ADR 인덱스 (95개 결정) | [`docs/adr/README.md`](docs/adr/README.md) |
