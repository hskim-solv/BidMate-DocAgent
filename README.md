...
- **Embedding 디폴트 = MiniLM-L12-v2 ([ADR 0019](docs/adr/0019-embedding-default-stays-minilm.md) 잠금)** — 1차 측정(MiniLM-L12-v2 vs multilingual-e5-base, 2026-05-11): full agentic 파이프라인에서는 0pp Δ, naive_baseline에서는 e5-base가 accuracy +18.8pp — metadata-first filtering이 임베딩 선택에 robust함을 실증. 2차 사이클 Phase 1.2 (이슈 #174, 2026-05-12) 3-of-4 측정 완료: `multilingual-e5-large-instruct`와 `KoSimCSE-roberta-multitask`도 `full` 파이프라인에서 0pp Δ (메트릭이 bit-identical) — 한국어 특화 모델까지 포함해 "0pp on full" 패턴이 falsify되지 않음. `BAAI/bge-m3`는 `torch 2.2.2`(CVE-2025-32434이 `torch >= 2.6` 요구) 블로커로 Phase 1.3로 이월. ADR 0019 재오픈 조건 3 (full에서 ≥+5pp + non-overlapping CI)은 미충족 → 디폴트 그대로 유지. 측정 표·러너: [`docs/embedding-ablation.md`](docs/embedding-ablation.md).
...
`sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2`를 기본으로 채택. 채택 이유: 한국어/영어 multilingual 지원, L12 경량(~120MB), API 의존 0, 로컬 재현성. **검증된 한계**: Phase 1.2 ablation (2026-05-12, n=42)에서 `multilingual-e5-large-instruct`와 `KoSimCSE-roberta-multitask` 둘 다 `full` 파이프라인에서 0pp Δ — 메트릭이 bit-identical하게 일치. 임베딩 선택 robustness는 ADR 0002 metadata-first 설계의 empirical consequence임이 한국어 특화 모델까지 확장되어 falsify되지 않음. `BAAI/bge-m3`는 `torch >= 2.6` 환경에서 Phase 1.3 재실험 예정.
