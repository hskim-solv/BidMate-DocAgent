...
- **Embedding 모델 ablation — 1차 결과 측정 완료** — `paraphrase-multilingual-MiniLM-L12-v2`(2019) vs `intfloat/multilingual-e5-base` 비교 결과: `full` 파이프라인은 두 모델 모두 동일(metadata-first가 dense를 우회), `naive_baseline`에서만 accuracy +18.8pp(0.656→0.844), groundedness +11.9pp 향상. 측정 절차는 [`docs/embedding-ablation.md`](docs/embedding-ablation.md), 런너는 [`scripts/run_embedding_ablation.py`](scripts/run_embedding_ablation.py). BGE-M3 / e5-large / KoSimCSE 등 대형 모델 비교는 디스크 확보 후 follow-up.
