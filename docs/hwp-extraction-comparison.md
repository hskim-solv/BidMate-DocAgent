...
샘플 N = TBD 개 (사용자 로컬), HWP 5.x.

| 차원 | hwp5txt | libreoffice → visual-v2 | Δ / 비고 |
|---|---|---|---|
| 본문 char 길이 (median) | TBD | TBD | TBD |
| 표 재구성 (table_count, 5 건 표본) | 0 (구조적) | TBD | TBD |
| OCR fallback page 비율 | n/a | TBD / N | text layer 부재 시 |
| 인코딩 오류 (자모 분리, ㈜/₩/㎡ 손상) 건수 | TBD | TBD | TBD |
| Latency / doc (median, p95 ms) | TBD | TBD (convert + parse) | TBD |
| 실패율 (status != ok) | TBD / N | TBD / N | TBD |
...
- 선행 문서: [`docs/visual-ingestion-v2.md`](visual-ingestion-v2.md) "What this is NOT" §
