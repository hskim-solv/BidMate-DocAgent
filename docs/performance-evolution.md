...
> 이 문서는 BidMate-DocAgent 의 평가 지표가 시간에 따라 어떻게 변화했는지를 기록한다. 현재 공개 저장소는 작은 fixture smoke를 재현성 확인용으로만 유지하고, 실제 성능 추이는 private/internal aggregate 문서에서 관리한다.
...
`naive_baseline` 은 ADR 0001 ("preserve extractive floor") 에 의해 **의도적으로 불변**이다. 아래는 aggregate report history 의 요약 통계:
...
`agentic_full` historical time-series 는 retired public aggregate artifacts에서 파생됐으며, 현재는 private/internal aggregate 문서에서만 성능 추이를 관리한다.
...
| [#472](https://github.com/hskim-solv/BidMate-DocAgent/pull/472) | push-per-commit → 일 1회 cron | aggregate report 노이즈 감소 |
| [#570](https://github.com/hskim-solv/BidMate-DocAgent/issues/570) | 평가셋 n=42 → n=100 | CI 반폭 ×0.65 수축, 미세 ablation 효과 검출 가능 |

---

> 이 문서의 과거 수치는 `reports/eval_summary.json` 및 private/internal aggregate 문서에서 파생됩니다. 상세 STAR 회고는 [docs/rag-challenges-solved.md](rag-challenges-solved.md) 를 참조하세요.
