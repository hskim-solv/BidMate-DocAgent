...
2. **Fall-back 카운트**: synthesis가 가드 ([`rag_synthesis.py:97-105`](../rag_synthesis.py)
   ...
   metadata).
3. **인용 grounding 회귀 카운트** ([`docs/real-data/real-data-failure-taxonomy.md`](../real-data/real-data-failure-taxonomy.md))
...
5. **Trace 시각 검증** (선택): [`docs/operations/observability.md`](./observability.md) 에
...
# 에서 직접 읽어야 합니다 — docs/operations/observability.md 참고.
...
1. **Trace backend** — `BIDMATE_TRACE_BACKEND=langfuse` (또는 otel) 로 candidate run을
   ...
   `output_tokens` 를 집계 ([`docs/operations/observability.md`](./observability.md) 참고).
...
- [`docs/agentic/answer-policy.md`](../agentic/answer-policy.md) — schema_version 호환성 가드 코드 위치.
...
- [`docs/real-data/real-data-failure-taxonomy.md`](../real-data/real-data-failure-taxonomy.md) — Step 2 의 6 카테고리 회귀 카운트 정의.
