...
본 노트 §3.3 표에는 **aggregate Δ 만** 옮겨 적는다. per-case predictions / chunk IDs / 원문은 절대 commit 하지 않는다 ([docs/private-100-doc-experiments.md](../private-100-doc-experiments.md) 의 commit boundary).
...
- **BGE-M3 환경 의존성** — [ADR 0021](../adr/0021-bge-m3-completes-phase-1-3.md) 와 [docs/embedding-ablation.md](../embedding-ablation.md) 가 `torch < 2.6` 제약을 명시. #151 이 환경 이슈로 ship 되지 않으면 §6 Stage 2 의 BGE-M3 row 자체가 측정 불가 — 이 경우 #152 는 무기한 deferred.
...
- [docs/ablation-results.md](../ablation-results.md) — 공개 synthetic ablation rows (hybrid_bm25 k-sweep, bm25_extra).
- [docs/private-100-doc-experiments.md](../private-100-doc-experiments.md) — ADR 0005 commit boundary.
- [docs/embedding-ablation.md](../embedding-ablation.md) — 모델 size / latency anchor.
