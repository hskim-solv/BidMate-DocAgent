# 152: SPLADE / ColBERT late-interaction 타당성(feasibility)

- **Status**: investigation (미채택; ADR 0025 가능성에 선행)
- **Date**: 2026-05-12
- **Deciders**: retrieval owner, eval owner
- **Related**: [#152](https://github.com/hskim-solv/BidMate-DocAgent/issues/152) (이 문서), [#119](https://github.com/hskim-solv/BidMate-DocAgent/issues/119) (hybrid BM25 부모), [#149](https://github.com/hskim-solv/BidMate-DocAgent/issues/149) (RRF k sweep), [#150](https://github.com/hskim-solv/BidMate-DocAgent/issues/150) (Korean STOPWORDS), [#151](https://github.com/hskim-solv/BidMate-DocAgent/issues/151) (BGE-M3, open), [ADR 0010](../adr/0010-hybrid-bm25-dense-retrieval-rrf.md), [ADR 0021](../adr/0021-bge-m3-completes-phase-1-3.md)
...
| 지표(Metric) | Base (dense) | Head (hybrid_bm25) | Δ |
|---|---|---|---|
...
## 3. Gap 정량화(quantification)

### 3.1 Sweep 설계 (2×2 최소, four cells)
...
### 3.3 결과(Results) (private 21-case)
...
### 3.4 의사결정 규칙(Decision rule, noise-aware)
...
| Variant | chunk 당 bytes | vs dense baseline | N=10,000 chunks 기준 |
|---|---|---|---|
...
## 5. SPLADE vs BGE-M3 sparse head 등가성(equivalence) (desk research)

| 축(Axis) | SPLADE-v2 (`naver/splade-cocondenser-ensembledistil`) | BGE-M3 sparse head |
|---|---|---|
...
|---|---|---|
| **BGE-M3 가 gap 닫음** (§7 hand-off 기준 충족) | #152 를 no-go 로 close. ADR 0021 + #151 PR 으로 커버. | #152 를 close. #151 단독 ship. |
| **BGE-M3 가 gap 못 닫음** | **ADR 0025 를 open** — scope: ColBERTv2 + PLAID late interaction 한정. SPLADE 분기는 §5 verdict 로 영구 배제. | 잔여 gap 을 문서화; defer; 재평가(re-evaluate) 시 더 저렴한 late-interaction 기법 (예: COIL, ALIGNER) 등장 시 재검토. |

각 cell 은 verb-first action — close / open / document / defer — 으로 명시.

## 7. #151 로의 hand-off 기준(criteria)
...
- **n=21 noise floor** — single case flip = 0.048, gate (0.05) 와 인접. §3.4 sign rule 로 완화(mitigate).
...
- **BGE-M3 환경 의존성** — [ADR 0021](../adr/0021-bge-m3-completes-phase-1-3.md) 와 [docs/eval/embedding-ablation.md](../eval/embedding-ablation.md) 가 `torch < 2.6` 제약을 명시. #151 이 환경 이슈로 ship 되지 않으면 §6 Stage 2 의 BGE-M3 row 자체가 측정 불가 — 이 경우 #152 는 무기한 deferred.
...
- [rag_pipeline_presets.py:40](../../rag_pipeline_presets.py:40) — `RRF_K = 60` 기본값.
