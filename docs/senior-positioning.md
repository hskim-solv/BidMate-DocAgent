...
| 시그널 | 어디서 확인하나 |
|---|---|
| 아키텍처 결정이 **사후 합리화가 아닌 기록된 결정**으로 남아있다 | [`docs/adr/`](./adr/README.md) — 32개 ADR (25 accepted / 7 proposed), status-tracked, supersession chains 명시 |
...
| [0032](./adr/0032-eval-saturation-routed-subset.md) | accepted | Eval-set saturation hypothesis + routed-subset measurement surface as falsifier for ADR 0019 deferral (extends 0019 / 0021; preserves 0001 / 0002; adds `agentic_full_routed` ablation preset; spread ≥ +3pp on routed subset triggers 0019 re-open) | ADR 0019 "0pp on full" 패턴이 시스템 robustness가 아니라 metadata-first 흡수에 의한 saturation 신호일 가능성을 falsify 가능한 측정 surface로 변환. 2026-05-13 첫 실행 결과: 5 임베딩 × routed-subset(n=11) 측정, spread = **0.0pp** → `saturation_cross_validated`. MiniLM default lock이 metadata-first 마스킹을 넘어 empirically justified. 외부 적대적 메타비판([PR #487](https://github.com/hskim-solv/BidMate-DocAgent/pull/487) §4.4-B2)에 evidence-backed 응답. |
