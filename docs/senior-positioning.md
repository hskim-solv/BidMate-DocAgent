...
| [0032](./adr/0032-eval-saturation-routed-subset.md) | proposed | Eval-set saturation hypothesis + routed-subset measurement surface as falsifier for ADR 0019 deferral (extends 0019 / 0021; preserves 0001 / 0002; adds `agentic_full_routed` ablation preset; spread ≥ +3pp on routed subset triggers 0019 re-open) | ADR 0019 "0pp on full" 패턴이 시스템 robustness가 아니라 metadata-first 흡수에 의한 saturation 신호일 가능성을 falsify 가능한 측정 surface로 변환. `routed_subset.jsonl` (n ≥ 10, multi-turn + 다문서 비교 ambiguity) + 신규 `agentic_full_routed` preset + 5 임베딩 ablation으로 spread ≥ +3pp 시 ADR 0019 re-open trigger, < +3pp 시 saturation cross-validated 결론. 외부 적대적 메타비판([PR #487](https://github.com/hskim-solv/BidMate-DocAgent/pull/487) §4.4-B2)에 evidence-backed 응답이자, ADR 0019의 measurement-gated 정신 자체의 *testability*를 검증하는 메타-거버넌스 작업. |
