...
### 3. [실패 분류로 백로그 생성하기](./2026-05-failure-taxonomy-backlog/)

발견한 실패를 ad-hoc하게 수정하는 대신 **분류 → 카테고리별 root cause → 코드 위치 → Impact/Effort grid → GitHub 이슈**로 매핑하는 방법론. C6 false abstention → 이슈 #69, C4 follow-up loss → 이슈 #71, C2 ambiguity → 이슈 #72 매핑 walk-through. taxonomy artifact가 어떻게 *백로그 우선순위*까지 자동으로 결정하는지의 5가지 원칙.

### 4. [Observability를 baseline 깨지 않고 추가하는 패턴](./2026-05-observability-fail-closed/)

LangFuse·OTel·미래의 백엔드까지 *같은 한 줄*로 들어오게 만드는 pluggable backend registry. **결정적 noop default + lazy import + fail-closed 예외 처리** 세 장치로 ADR 0001(baseline) / ADR 0005(eval split) / ADR 0003(answer contract) 어느 것도 깨지 않는다. judge → synthesis → trace에 같은 패턴을 세 번째로 적용한 사례.

### 5. [외부 시니어 리뷰 → 사실 정정 + ADR 매트릭스](./2026-05-external-review-followup/)

외부 코드 리뷰의 권고 50%+가 이미 ADR로 결정돼 있었거나 측정 후 명시적으로 거부된 항목이었다. *사실 정정 매트릭스*(놓친 구현 11개) + *측정-게이트 거부 매핑*(8개 권고를 7개 ADR에 매핑) + *진짜 미구현 영역의 PR 시퀀스*를 한 문서에 모아 같은 권고가 재유입될 때 즉시 회신 가능하게 만든다. ADR 0019 → 0021의 *deferred-then-closed loop* 패턴이 어떻게 외부 리뷰 대응의 핵심 도구가 되는지의 walk-through.
