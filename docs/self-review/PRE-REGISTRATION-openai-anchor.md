...
## Outcome B (2026-05-21) — operator blind 재채점: (c) 기각, (b)가 최선의 설명

위 Outcome 의 (c) lean 을 정정한다. 규약대로 위 (c) 기록은 **보존**하고, 본 절이 그 위에 정정을 누적한다 (사후 삭제 금지).

- **방법**: 오염 없는 fresh subagent 에 full SKILL.md 채점표 + 동결 스냅샷 `ae08bc6c…` 신호만 제공 (consensus·가설 미노출), blind 채점.
- **결과**: blind operator = `△ ✗ ✓ ✓ △`. mechanical consensus (`△ ✗ ✓ ✓ ✓`) 와 **4/5 일치**, #5 만 갈림.
- **#5 차이는 (c) 아님**: blind operator 가 full SKILL.md (5-A `null`→△ + 5-B ✓ → 결합 △) 를 따른 결과. stub 은 5-A 미구현 (5-B 만) 이라 ✓. 즉 blind operator 가 stub 보다 **더 충실** — axis_5 에 실제 spec/구현 gap (5-A 처리) 이 있음을 드러냄.

**판정:**

- (a) underspecified: 4 개 날카로운 축 (#1–#4) 에서 stub·openai·blind operator 셋 다 수렴 → **기각**. (단 axis_5 는 5-A gap 으로 미해결.)
- **(c) operator inflation: 기각.** blind operator 가 현재 데이터에서 mechanical 과 일치 (Q2 의 1✓4△ 가 아니라) → 절차는 현재 데이터에 충실, 부풀리지 않음.
- **(b) 시점 드리프트: 최선의 설명.** Q2 1✓4△ 는 복원불가 Q2-시점 신호 때문. axis_3 은 `null`→△ 로 깔끔히 (b) 증명.

**필수 caveat (과대주장 차단):**

1. **Q2-시점 절차 충실성은 직접 검증 불가 — 추론이다.** blind operator 는 *현재* 신호로 채점했지 복원불가한 Q2-시점 신호로 재현한 게 아니다. 직접 증명된 건 "절차는 *현재* 데이터에 충실" 까지. axis_3 외 다른 축이 Q2-시점에 더 나빴다는 건 **개연성** 이지 직접 검증 아님. 그래서 결론은 "(c) 이 검정으로 미지지 + (b) 가 충분한 설명" 이지, **"red-team 비판이 틀렸다" 까지 단정하면 과함.**
2. **κ=1.0 과대 회수.** openai-vs-stub κ=1.0 은 openai 프롬프트에 stub 의 *축소* rubric (axis_5 = 5-B 만) 을 넣어 #5 일치가 구조적으로 보장된 부분이 있다. 정확한 진술은 "full rubric 완전 일치" 가 아니라 **"4 개 날카로운 축 operationalize + axis_5 (5-A) 에 실제 gap"**.
3. **self-verification 구조 잔존.** (b) 결론은 자기채점 절차를 면죄하는 방향이다. 사전등록·blind·κ 회수로 편향을 꽤 통제했으나, 실험 설계·해석을 같은 주체 (이 시스템) 가 했다는 구조는 남는다 — **외부 (사람/타 주체) 재현이 궁극의 통제.**
