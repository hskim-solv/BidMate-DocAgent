...
- **(e) operator opt-in 경로.** `gate-evidence` CLI 에 `--run-validation` flag 를 추가하고 handler 가 `write_active_gate_evidence(..., run_validation=args.run_validation)` 로 전달한다 → operator 가 토글을 실제로 켤 수 있다. active-loop 자동 호출(`agent_loop.py:11668`)은 기본 off 유지 — 자율 루프의 비결정성/race 를 피해 자동 검증은 opt-in CLI 로 한정한다.

**범위 한정 (one concern):** 본 ADR 은 `write_active_gate_evidence` 의 evidence gate 한 경로(+그 operator-facing `gate-evidence` CLI)만 강화한다. 나머지 3개 gate 경로(`_active_role_status_ok` 직접 호출처 / codex `status_for_gate` / exit-code gate), 그리고 mktemp 완전격리(ADR 0086 Deferred output-isolation)는 **follow-up 으로 분리**한다.
...
- (+) `--run-validation` CLI flag 로 operator 가 토글을 켤 수 있다 — 초기 구현의 dead-parameter(어느 호출처에서도 미전달) 결함을 adversarial review(P2)가 잡아 CLI wiring 을 추가. `repo_root`→`cwd` 전달로 검증이 caller cwd 가 아닌 gate worktree 에서 실행된다.
...
- `gate-evidence --run-validation` flag 가 parser 에 노출되고 handler 가 전달하는지 (operator opt-in 경로).
- `repo_root` 가 `run_validation_commands` 의 `cwd` 로 전달되는지 (gate worktree 에서 실행).

<!-- verifies-key: scripts/agent_loop.py:_gate_validation_signal -->
<!-- verifies-key: scripts/agent_loop.py:run_validation -->
<!-- verifies-key: scripts/agent_loop.py:--run-validation -->
