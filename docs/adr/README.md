...
| [0058](./0058-phase35-mode-winner.md) | accepted | Phase 3.5 mode-winner decision — **Scenario A applied: switch `retrieval_backend` default from `dense` to `hybrid` (RRF k=60 over BGE-M3 dense + BM25)** for `agentic_full` + `metadata_first` presets; `naive_baseline` stays `dense` (ADR 0001 invariant byte-identical); evidence = kordoc 26k measurement (n=221) with `chunk_recall@10 +0.052 SIG / MRR +0.110 SIG / ndcg@10 +0.065 SIG` overall + per-category SIG wins on multi_hop/distractor_heavy/long_context; latency 1.35x (757ms vs 559ms p50); m3 multi-channel deferred to cloud-GPU follow-up (16GB Apple Silicon unified-memory crash); supersedes Phase 3 PR #956 (runner bug retraction, fixed PR-H #994) + Phase 3.5 PR #966 1차 csv_text fallback (chunk count retraction, fixed PR #966 closeout); closes issue #981 + #982 + #997 |
