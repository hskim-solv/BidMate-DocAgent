...
| [0040](./0040-react-agent-loop-additive-preset.md) | accepted | ReAct agent loop as fourth additive pipeline preset (`agent_react`); `Planner` Protocol extends ADR 0020 4-axis pluggability to 5-axis; opt-in via `BIDMATE_PLANNER_BACKEND=anthropic` (default `static` keeps CI deterministic); preserves ADR 0001/0003/0024 invariants; closes issue #673 |
| [0041](./0041-agent-budget-cap-contract.md) | accepted | Agent budget cap contract for the `agent_react` loop — explicit caps on iteration count (`BIDMATE_AGENT_MAX_ITERATIONS=4`), latency, and token telemetry (extends ADR 0015); abstain path on budget hit; CI deterministic; closes issue #673 |
