...
| [0102](./0102-parser-element-micro-eval-wiring-surface.md) | accepted | Parser element micro-eval v0 를 reviewer-facing wiring regression surface 로 승격 — metadata/OCR/table/text-layer element stream 검색 가능성, path alias scoring, textless aggregate 를 검증한다. 허용 claim 은 parser element wiring/searchability 회귀 여부에 한정하며 parser 품질·OCR 정확도·real100_v2 성능 개선·canonical ingestion 교체 claim 은 금지한다. |
