...
| [0066](./0066-codex-pr-adversarial-review.md) | proposed | Local Codex adversarial pre-commit review loop — PR open/sync GitHub Actions workflow를 제거하고, `.githooks/pre-commit`이 load-bearing staged 변경에 한해 `scripts/run_codex_adversarial_precommit.py`를 실행한다; 트리거 SSoT는 `scripts/_governance.py LOAD_BEARING_PATHS`; review 대상은 staged diff(`git diff --cached`)이며 private eval path guard(ADR 0005)가 먼저 돈다; 기본 2회 attempt 모두 `approve`여야 commit 통과, attempt별 기본 timeout은 900초; `needs-attention`/parse error/companion failure/timeout은 block; artifact는 git-dir 내부 `codex-adversarial-precommit/` local-only; PR comment/check-run evidence는 제거하고 deterministic CI와 LLM critique를 분리; closes issue #1126 |
...
| [0089](./0089-codex-precommit-infra-failure-nonblocking.md) | proposed | pre-commit Codex adversarial review — infra 실패 비차단 (amends [0066](./0066-codex-pr-adversarial-review.md)). `run_codex_adversarial_precommit.py`가 codex **실행 실패(infra: auth/refresh-token race·network·timeout·파싱불가·companion부재 = verdict 못 얻음)**와 **실제 non-approve verdict**를 구분 못 하고 둘 다 커밋 hard-block → codex 인증 lapse 시 모든 load-bearing 커밋 차단(#1688 작업 중 `--no-verify` 강제 실측). 수정: `infra_failure = (rc != 0) or (verdict == "error")` → WARN + 커밋 허용(적대 리뷰 권위는 CI/pre-push에 위임), `rc==0 + non-approve verdict`만 차단(ADR 0066 핵심 보존). companion 미설치도 비차단. phase는 pre-commit 유지(pre-push 이동 검토 후 보류 — index-손상 논거는 재현 실패로 철회). closes #1689 |
