...
| [0040](./0040-react-agent-loop-additive-preset.md) | accepted | ReAct agent loop (`agent_react` preset) as additive ablation on top of `agentic_full`; `LLMPlanner` + `ReActOrchestrator` + `VerifyGrounding` tool; ADR 0001 naive_baseline invariant preserved; closes issue #673 |
| [0041](./0041-agent-budget-cap-contract.md) | accepted | Agent budget cap contract for `react_loop`: `REACT_MAX_ITERATIONS` (default 3) hard ceiling prevents unbounded LLM calls; `abstain` is the forced exit; verifier feedback injection (issue #679) fits within cap; ADR 0040 + ADR 0003 compatible |
