...
`eval/case_proposer.py`가 judge backend dispatch 패턴을 mirror:
...
- ADR 0011 "stub-default + opt-in live" 패턴의 1 추가 적용(현재: 0011 합성, 0013 관측, 0017 메타데이터 추출, 0023 HyDE, 0027 LoRA, 0028 보안 screen, 0029 case proposer). reviewer는 동일 shape 반복 관찰 → additive-pluggable idiom이 프로젝트 default.
...
- ADR 0005 allowlist에 파일 1 추가(`reports/proposed/proposer.aggregate.json`). 기존 private aggregate-only 예외 패턴을 mirror.
...
- ADR 0001 naive baseline golden(`tests/data/naive_baseline_top_k.json`) byte-identity. proposer는 `eval/real_config.local.yaml`(private)만 touch, public fixture smoke config는 절대 안 함.
- ADR 0003 답변 계약. proposer는 `run_rag_query` upstream; eval *입력* 생산, answer 출력 아님.
- ADR 0004 결정적 검증기. 공개 fixture CI는 proposer 절대 invoke 안 함(`pr-eval.yml` 또는 `make smoke`에 `make case-propose` 없음).
