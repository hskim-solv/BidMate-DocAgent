...
- **Date**: 2026-05-20
- **Deciders**: hskim-solv
- **Related**: CLAUDE.md `## Non-goals`, ADR 0001 (naive baseline byte-identical), ADR 0005 (eval public/private split), ADR 0012 (LLM judge on public synthetic), ADR 0011 (pluggable LLM synthesis), ADR 0023 (HyDE), ADR 0017 (LLM metadata extraction)
...
(b) ADR 0005/0012 의 비공개 RFP 데이터 보호. 이 ADR 은 (a)(b) 를 깨지 않으면서
...
- **쉬워짐**: LlamaParse/Docling 인덱스타임 파싱, Cohere Rerank, 외부 dense
  embedding, 강한 LLM-judge (public synthetic 한정) 를 ADR 게이트만 통과하면
...
- **남는 결정**: 비공개 데이터의 외부 전송을 허용할지는 미해결 — 필요 시 ADR 0005/
  0012 를 supersede 하는 별도 ADR 에서 다룬다.

## Alternatives considered

- **전면 해제 (데이터 전송 포함)**: ADR 0005/0012 를 동시에 supersede. 거부 —
